...
public static class Endpoints
{
    ...
    private static IResult GetJobTypes()
    {
        var jobTypes = Enum.GetValues<JobTypes>()
            .Where(jt => jt != JobTypes.None)
            .Select(jt => KeyValuePair.Create(jt, jt.GetDisplayName()))
            .ToArray();

        return Results.Ok(jobTypes);
    }
    ...
    private static async Task<IResult> GetJobsAsync(ISchedulerFactory schedulerFactory)
    {
        ...
        return Results.Ok(records.ToArray());
    }
    ...
    private static async Task<IResult> GetContainersAsync(IDockerClient dockerClient, IConfiguration configuration)
    {
        ...
        return Results.Ok(containerRecords.ToArray());
    }
